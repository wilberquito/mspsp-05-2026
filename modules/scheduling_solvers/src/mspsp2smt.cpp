...
int main(int argc, char **argv)
{
	...
	Arguments<ProgramArg> *pargs = new Arguments<ProgramArg>(
		...
		// Program options
		{
			arguments::bop("", "implied1", IMPLIED1, false,
						   "Add the implied contraint: the number of activities requiring skill 'l' at a particular time is not greater than the number of resources that master 'l'. Default: 0."),

			arguments::bop("", "implied2", IMPLIED2, false,
						   "Add the implied contraint: the number of activities running at a particular time is not greater than the number of resources. Default: 0."),

			arguments::bop("", "implied3", IMPLIED3, false,
						   "Add the implied contraint: for any combination of skills (dominance filtering applies), the number of activities running at a particular time is not greater than the number of resources that master those skills. Default: 0."),
			...
			arguments::iop("M", "makespan", MAKESPAN, -1,
						   "Makespan to use in the encoding. Default: -1."),
			...
			arguments::bop("", "symbreak", SYMBREAK, false,
						   "Add a symetry breaking contraint which set a preference order on the use of identical resources. Default: 0."),
			...
			arguments::sop("E", "encoding", ENCODING, "time", {"time", "timegerarquic", "timecomb", "timecomb2", "taskcomb2", "nbddtime", "timemaxsat", "timeopb", "timesat", "timepb"},
	...
	*/
	...
	else if (pargs->getStringOption(ENCODING) == "timemaxsat")
		encoding = new MSPSPTimeMaxSATEncoding(instance, sargs, pargs);
	else if (pargs->getStringOption(ENCODING) == "timesat")
		encoding = new MSPSPTimeSATEncoding(instance, sargs, pargs);
	else if (pargs->getStringOption(ENCODING) == "timeopb")
		encoding = new MSPSPTimeOPBEncoding(instance, sargs, pargs);
	else if (pargs->getStringOption(ENCODING) == "timepb")
		encoding = new MSPSPTimePBEncoding(instance, sargs, pargs);
	...
	if (sargs->getBoolOption(OUTPUT_ENCODING))
	{
		...
		// Encode the sat versions of the problem
		if (pargs->getStringOption(ENCODING) == "timesat") {
			int makespan = pargs->getIntOption(MAKESPAN);
			if (makespan < 0) {
				cout << "Makespan not specified for timesat encoding, using upper bound as makespan. This may lead to unexpected behavior." << endl;
				makespan = UB;
			}
			f = ((MSPSPTimeSATEncoding *) encoding)->encode_sat(makespan, LB, UB);
		} else {
			f = encoding->encode(LB, UB);
		}
	...
	else
	{
		...
		{
			opti->setAfterSatisfiabilityCall([=](int lb, int ub, Encoder *encoder)
											 { BasicController::afterSatisfiabilityCall(lb, ub, encoder); });
			opti->setAfterNativeOptimizationCall([=](int lb, int ub, Encoder *encoder)
												 { BasicController::afterNativeOptimizationCall(lb, ub, encoder); });
		}

		if (sargs->getBoolOption(PRINT_CHECKS))
			opti->setOnNewBoundsProved([=](int lb, int ub)
									   { BasicController::onNewBoundsProved(lb, ub); });

		if (sargs->getBoolOption(PRODUCE_MODELS) && sargs->getBoolOption(PRINT_NOOPTIMAL_SOLUTIONS))
			opti->setOnSATSolutionFound([=](int &lb, int &ub, int &obj_val)
										{ BasicController::onSATSolutionFound(lb, ub, obj_val, encoding); });
