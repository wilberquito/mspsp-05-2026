...
namespace smtapi
{
	...
	const std::vector<PBconstraint> &SMTFormula::getPBconstraints() const
	{
		return pbconstraints;
	}

	const PBgoal * SMTFormula::getPBgoal() const
	{
		return pbGoal;
	}

	const int SMTFormula::getNPBconstraints() const
	{
		return pbconstraints.size();
	}
	...
	bool SMTFormula::_bExists(const std::string &name) const
	{
		return mapBoolVars.find(name) != mapBoolVars.end();
	}
	...
	boolvar SMTFormula::newBoolVar(const std::string &s)
	{
		...
		if (_bExists(s))
		{
			std::cerr << "boolVar " << s << " already exists" << std::endl;
			exit(BADBOOLEANVAR_ERROR);
		}
#endif
		boolvar x;
		x.id = ++nBoolVars;
	...
	boolvar SMTFormula::newBoolVar(const std::string &s, int i1)
	{
		...
		if (_bExists(s2))
		{
			std::cerr << "boolVar " << s2 << " already exists" << std::endl;
			exit(BADBOOLEANVAR_ERROR);
		}
#endif
		boolvar x;
	...
	bool SMTFormula::bvarExists(const std::string &var) const
	{
		return _bExists(var);
	}

	bool SMTFormula::bvarExists(const std::string &var, int i1) const
	{
		return _bExists(ssubs(var, i1));
	}

	bool SMTFormula::bvarExists(const std::string &var, int i1, int i2) const
	{
		return _bExists(ssubs(var, i1, i2));
	}

	bool SMTFormula::bvarExists(const std::string &var, int i1, int i2, int i3) const
	{
		return _bExists(ssubs(var, i1, i2, i3));
	}
	...
	boolvar SMTFormula::_bvar(const std::string &s) const
	{
#ifndef NDEBUG
		if (!_bExists(s))
		{
			std::cerr << "boolVar " << s << " does not exist" << std::endl;
			exit(BADBOOLEANVAR_ERROR);
		}
#endif
		return mapBoolVars.at(s);
	}
	...
	{
#ifndef NDEBUG
		// TODO: only works with SAT, not intvars
		for (const literal &l : c.v)
		{
			assert(l.v.id <= nBoolVars && l.v.id >= 1);
		}
#endif
		clauses.push_back(c);
	}
	...
	void SMTFormula::addPBconstraint(const clause &c)
	{
#ifndef NDEBUG // Assert the clause does not contain smt literals
		for (const literal &l : c.v)
			assert(!l.arith);
#endif
		std::vector<int> Q(c.v.size(), 1);
		addPBconstraint(Q, c.v, ">=", 1);
	}

	void SMTFormula::addPBgoal(const std::vector<literal> &X)
	{
#ifndef NDEBUG
		assert(!X.empty());
		for (const literal &l : X)
			assert(!l.arith);
#endif
		std::vector<int> Q(X.size(), 1);
		PBgoal *goal = new PBgoal(Q, X);
	...
	void SMTFormula::addPBAndReification(const literal &y, const std::vector<literal> &L)
	{
		addPBCardinalityReification(y, L, L.size());
	}

	void SMTFormula::addPBOrReification(const literal &y, const std::vector<literal> &L)
	{
		addPBCardinalityReification(y, L, 1);
	}

	void SMTFormula::addPBCardinalityReification(literal y, std::vector<literal> L, int K)
	{
		// FYI: y <-> C
		// y <-> Q · X >= K
		...
		// FYI: y -> C
		// y -> Q · X >= K
		// K · \neg y + \sum_{i}^{m} Q[i] · L[i] >= K
		// K · \neg y + 1 · L_{0} + ... + 1 · L_{m-1} >= K

		int M = L.size();
		std::vector<int> Q(M, 1);
		Q.push_back(K); // TODO: check the correctness of this (maybe it is always 1)
		L.push_back(!y);
		addPBconstraint(Q, L, ">=", K);
		...
		{
			L[i] = !L[i]; // Negate all literals except the last one (which is y)
		}
		addPBconstraint(Q, L, ">=", (-K + 1 + M));
	}

	void SMTFormula::addImpliedPBCardinality(literal y, std::vector<literal> L, int K)
	{
		// FYI: y -> C
		// y -> Q · X >= K
		// K · \neg y + \sum_{i}^{m} Q[i] · L[i] >= K
	...
	void SMTFormula::addPBconstraint(const std::vector<int> &Q, const std::vector<literal> &X, std::string op, int K)
	{
		int op_code = 0; // Default is >=
		if (op == "<=")
			op_code = 1;
		else if (op == "=")
			op_code = 2;

		addPBconstraint(Q, X, K, op_code);
	}

	void SMTFormula::addPBconstraint(const std::vector<int> &Q, const std::vector<literal> &X, int K, int op)
	{
		assert(Q.size() == X.size());
		PBconstraint pb(Q, X, K, op == 2);

		// Normalize the constraint: remove negated literals and make it >= if it was <=
		if (op == 1)
			pb.k = -pb.k;
		for (int i = 0; i < pb.q.size(); i++)
		{
			if (op == 1)
				pb.q[i] = -pb.q[i];
			if (pb.x[i].sign == false)
			{
				pb.x[i].sign = true;
				pb.k -= pb.q[i];
				pb.q[i] = -pb.q[i];
	...
	void SMTFormula::addALK(const std::vector<literal> &v, int K, CardinalityEncoding enc)
	{
		...
		else
		{
			...
			{
				if (K > n / 2)
				{ // AMK encoding of the negated literals is smaller
					std::vector<literal> v2;
					for (const literal &l : v)
						v2.push_back(!l);
	...
	void SMTFormula::addAMK(const std::vector<literal> &v, int K, CardinalityEncoding enc)
	{
		...
		else
		{
			...
			{
				if (K > n / 2)
				{ // ALK encoding of the negated literals is smaller
					std::vector<literal> v2;
					for (const literal &l : v)
						v2.push_back(!l);
					addALK(v2, n - K);
	...
	void SMTFormula::addImpliedALK(const literal &y, const std::vector<literal> &v, int K, CardinalityEncoding enc)
	{
		int n = clauses.size();
		// The ALK adds a collection of clauses (C_1, C_2, ..., C_m) enforcing that at least K literals in v are true
		// To enforce that if y is true then at least K literals in v are true,
		// we modify the added clauses to (C_1 ∨ ¬y, C_2 ∨ ¬y, ..., C_m ∨ ¬y)
		addALK(v, K, enc);
		for (int i = std::max(n - 1, 0); i < clauses.size(); i++)
		{
			clause & c = clauses[i];
			c.v.push_back(!y);
		}
	...
	void SMTFormula::addEK(const std::vector<literal> &v, int K)
	{
		...
		{
			if (K > n / 2)
			{ // EK encoding of the negated literals is smaller
				std::vector<literal> v2;
				for (const literal &l : v)
					v2.push_back(!l);
				addEK(v2, n - K);
			...
			for (int i = n; i >= 0; i--)
			{
				...
				for (int j = 0; j < w.size(); j++)
				{
					...
					if (wmod != 0)
					{
						...
						{
							if (!varWithCoefCreated[wmod])
							{ // Replace original variable by a half-reified aux variable
								boolvar v = newBoolVar();
			...
			for (int h = 0; h < n; h++)
			{
				...
				for (i = Dleft[h].begin(); i != Dleft[h].end(); i++)
				{
					for (j = Dright[h].begin(); j != Dright[h].end(); j++)
					{
						...
						if (sum > 0)
						{ // Clauses for sum==0 trivially satisfied
						...
						if (h > 0 && prevLevelHasCarry)
						{		   // If previous level never has carry, clauses trivially satisfied
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (!inputs.empty())
				{
					...
					else
					{
						...
						if (it == constructedYs[i].end())
						{ // If not constructed before, add half reifications and store
							y = newBoolVar();
							for (int j : inputs)
								addClause(!X[i][j] | y);
			...
			for (int j = 0; j < Q[i].size(); j++)
			{
				...
				for (int k = 0; k <= p; k++)
				{
					...
					else
					{
						...
						int firstBit;
						if (Tkm1)
						{ // If the bit of T in the previous bucket is 1, shift
							firstBit = 0;
