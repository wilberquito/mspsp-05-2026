...
bool nthBit(int x, int n){
	// returns true if the n-th bit of x is 1, false otherwise
	// The `>> n` is to shift n times to the right
	// The `& 1` is to get only the last bit
	return (x >> n) & 1;
}
...
int cardinality(int x){
	...
	int count = 0;
	// While x is not zero (otherwise it escapes the loop)
	while(x)
	{
