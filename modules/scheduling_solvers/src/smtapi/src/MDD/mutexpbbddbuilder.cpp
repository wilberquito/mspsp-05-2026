#include "PBMDDBuilder.h"
#include "util.h"



PBMDDBuilder::PBMDDBuilder(const vector<vector<int> > &Q,const vector<vector<boolvar> > &X, int K, bool longedges)
	: MDDBuilder(){
...
PBMDDBuilder::PBMDDBuilder(const vector<int> &Q,const vector<boolvar> &X, int K, bool longedges)
	: MDDBuilder(){
	...
	for(int x : X)
		this->X.push_back(vector<boolvar>(1,x));
...
PBMDDBuilder::~PBMDDBuilder(){
  for(const vector<MDD *> & layer : L_MDDs)
    for(MDD * m : layer)
      delete m;
}

MDD * PBMDDBuilder::buildMDD(){
	return MDDConstruction(0,K).mdd;
}

void PBMDDBuilder::mostrarL(int i_l) {
    int i=0,j=0,max=0;
    R_M rm;
...
int PBMDDBuilder::inf_sum(int possible_inf, int x){
...
void PBMDDBuilder::insertMDD(R_M rm_in,int i_l) {
    R_M act;
...
R_M PBMDDBuilder::searchMDD(int i_k,int i_l) {
	 vector<R_M> & layer = L[i_l];
...
R_M PBMDDBuilder::MDDConstruction(int i_l,int i_k) {
	...
	if(rm_new.mdd==NULL){
		...
		bool reusechild = longedges && mdds.size() == 1;
...
int PBMDDBuilder::getmax(const vector<int> & v){
 int max = INT_MIN;
 for(int i : v)
  if(i > max)
    max = i;
...
void PBMDDBuilder::createGraphviz(ostream & os, vector<vector<int> > * labels) const{
  /*map<MDD *,bool> ids;
  os << "digraph G {" << endl;
  ...
  os << "#";
   for(int i = 0; i < depth-1; i++){
    ...
    os << "{" << Q[depth-1][0];
    for(int j = 1; j < Q[depth-1].size(); j++)
      os << "," << Q[depth-1][j];
    os << "} <= [" << root->B << "," << root->Y << "]" << endl;


  root->createGraphviz(os,ids);
  os << "}" << endl;*/
}
