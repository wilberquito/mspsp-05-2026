...
SolvingArguments::SolvingArguments() : Arguments<SolvingArg>(
	...
	{"smtlib2","dimacs","wcnf","wcnfpre21","opb"},
	"Format to be used to output the encodings (if -E=1). Options: dimacs, smtlib2, wcnf, wcnfpre21, opb. For SMT encodings, smlib2 is required. Default: smtlib2."),
...
FileEncoder * SolvingArguments::getFileEncoder(Encoding * enc){
	...
	else if(fileformat=="wcnf"){
		fe = new WCNFFileEncoder(enc,solver);
		fe->setTmpFileName(fileprefix +".wcnf");
	}
	else if(fileformat=="wcnfpre21"){
		fe = new WCNFFileEncoderPre21(enc,solver);
		fe->setTmpFileName(fileprefix +".wcnf");
	}
	else if(fileformat=="opb"){
		fe = new OPBFileEncoder(enc,solver);
		fe->setTmpFileName(fileprefix +".opb");
	}
	else{
