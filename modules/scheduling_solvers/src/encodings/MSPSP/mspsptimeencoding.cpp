...
MSPSPTimeEncoding::MSPSPTimeEncoding(MSPSP * instance, SolvingArguments * sargs, Arguments<ProgramArg> * pargs) : MSPSPEncoding(instance) {
	this->sargs = sargs;
	this->pargs = pargs;
}

MSPSPTimeEncoding::~MSPSPTimeEncoding() {

}


SMTFormula * MSPSPTimeEncoding::encode(int lb, int ub){
	...
	for(int i = 1; i <= N; i++){
		for(int k = 0; k < R; k++) if(ins->usefulToActivity(k,i)){
			boolvar b = f->newBoolVar("act_res",i,k);
			vector<literal> v;
			v.push_back(!b);
			for(int l = 0; l < L; l++){
				if(ins->hasSkill(k,l) && ins->demandsSkill(i,l)){
					boolvar b2 = f->newBoolVar("act_res_skill",i,k,l);
					v.push_back(b2);
					f->addClause(!b2 | b); //Chanelling act_res act_res_skill
				}
			}
			//Each resource spends at most one skill in each activity
			f->addAMO(v,sargs->getAMOEncoding());

			//Chanelling act_res act_res_skill
			f->addClause(v);
		}
	}

	//Each activity has exactly K resources covering a skill of capacity K
	for(int i = 1; i <= N; i++){
		for(int l = 0; l < L; l++){
			vector<literal> v;
			for(int k = 0; k < R; k++){
...
void MSPSPTimeEncoding::setModel(const EncodedFormula & ef, int lb, int ub, const vector<bool> & bmodel, const vector<int> & imodel){
	int N = ins->getNActivities();
	int R = ins->getNResources();
	int L = ins->getNSkills();
...
bool MSPSPTimeEncoding::narrowBounds(const EncodedFormula & ef, int lastLB, int lastUB, int lb, int ub){
	int N = ins->getNActivities();
	if(ub <= lastUB){
...
void MSPSPTimeEncoding::assumeBounds(const EncodedFormula & ef, int lb, int ub, vector<literal> & assumptions){
	int N = ins->getNActivities();
