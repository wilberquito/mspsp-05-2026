...
MSPSPTimePBEncoding::MSPSPTimePBEncoding(MSPSP *instance, SolvingArguments *sargs, Arguments<ProgramArg> *pargs) : MSPSPEncoding(instance)
{
	this->sargs = sargs;
	this->pargs = pargs;
}

MSPSPTimePBEncoding::~MSPSPTimePBEncoding()
{
}

SMTFormula *MSPSPTimePBEncoding::encode(int lb, int ub)
{
...
void MSPSPTimePBEncoding::setModel(const EncodedFormula &ef, int lb, int ub, const vector<bool> &bmodel, const vector<int> &imodel)
{
	int N = ins->getNActivities();
	int R = ins->getNResources();
	int L = ins->getNSkills();
...
bool MSPSPTimePBEncoding::narrowBounds(const EncodedFormula &ef, int lastLB, int lastUB, int lb, int ub)
{
	int N = ins->getNActivities();
	if (ub <= lastUB)
	{
...
void MSPSPTimePBEncoding::assumeBounds(const EncodedFormula &ef, int lb, int ub, vector<literal> &assumptions)
{
	int N = ins->getNActivities();
...
MSPSPModel MSPSPTimePBEncoding::makeModel(const EncodedFormula &ef, const istream &bmodel) const
{
	vector<bool> model;
...
bool MSPSPTimePBEncoding::findChar(std::istream &model_str, char c) const
{
	char aux;
...
void MSPSPTimePBEncoding::readModel(istream &model_str, char c, vector<bool> &model) const
{
