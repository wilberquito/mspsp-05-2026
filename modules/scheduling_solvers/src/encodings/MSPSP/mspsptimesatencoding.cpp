...
MSPSPTimeSATEncoding::MSPSPTimeSATEncoding(MSPSP *instance, SolvingArguments *sargs, Arguments<ProgramArg> *pargs) : MSPSPEncoding(instance)
{
	this->sargs = sargs;
	this->pargs = pargs;
}

MSPSPTimeSATEncoding::~MSPSPTimeSATEncoding()
{
}

SMTFormula *MSPSPTimeSATEncoding::encode(int lb, int ub)
{
	return encode_sat(ub, lb, ub);
}

SMTFormula *MSPSPTimeSATEncoding::encode_sat(int makespan, int lb, int ub)
{
	// In debug mode check that makespan is smaller or equal to the ub
#ifndef NDEBUG
	if (makespan > ub)
	{
		cerr << "Warning: makespan " << makespan << " is greater than the upper bound " << ub << ". Consider increasing the upper bound or precomputing a better one." << endl;
	}
#endif
	if (makespan > ub)
		makespan = ub;
	...
	// Contradictions when time windows is empty
	for (int i = 0; i <= N + 1; i++)
	{
		int es = ins->ES(i);
		int ls = ins->LS(i, ub);
	...
	// Forces the latest dummy activity started at precomputed makespan
	f->addClause(f->bvar("s", N + 1, makespan));
...
void MSPSPTimeSATEncoding::setModel(const EncodedFormula &ef, int lb, int ub, const vector<bool> &bmodel, const vector<int> &imodel)
{
	int N = ins->getNActivities();
	int R = ins->getNResources();
	int L = ins->getNSkills();
...
bool MSPSPTimeSATEncoding::narrowBounds(const EncodedFormula &ef, int lastLB, int lastUB, int lb, int ub)
{
	int N = ins->getNActivities();
	if (ub <= lastUB)
	{
...
void MSPSPTimeSATEncoding::assumeBounds(const EncodedFormula &ef, int lb, int ub, vector<literal> &assumptions)
{
	int N = ins->getNActivities();
...
MSPSPModel MSPSPTimeSATEncoding::makeModel(const EncodedFormula &ef, const istream &bmodel) const
{
	vector<bool> model;
...
bool MSPSPTimeSATEncoding::findChar(std::istream &model_str, char c) const
{
	char aux;
...
void MSPSPTimeSATEncoding::readModel(istream &model_str, char c, vector<bool> &model) const
{
