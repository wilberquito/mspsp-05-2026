...
MSPSP::MSPSP(int nactivities, int nresources, int nskills){
	...
	extPrecs[0][0]=0;
	extPrecs[nactivities+1][nactivities+1]=0;
}
...
{
	// Provide the maximum value from 0 to UB - extPrecs[i][nactivities+1]
	return std::max(ES(i), UB - extPrecs[i][nactivities+1]);
}
...
void MSPSP::computeDominatedSets(){
	...
	// This refers to the empty set
	subsetsCard[0].push_back(0);

	// Number of resources that can cover at least one skill in each subset
	...
	{
		// Adding the combination to the corresponding cardinality list,
		// meaning that it has n = cardinality(c) skills in the set
		subsetsCard[util::cardinality(c)].push_back(c);
		// Computes the number of resources that can cover at least one skill in the combination
		for(int k = 0; k < nresources; k++)
		{
			for(int l = 0; l < nskills; l++)
			{
				// If the current skill l appears in the combination c
				// and resource k masters skill l
				// Notice the break, since we care about the at least one skill
