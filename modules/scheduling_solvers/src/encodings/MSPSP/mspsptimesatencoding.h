#ifndef MSPSPTIMESATENCODING_DEFINITION
#define MSPSPTIMESATENCODING_DEFINITION
...
class MSPSPTimeSATEncoding : public MSPSPEncoding {
private:
...
MSPSPTimeSATEncoding(MSPSP * instance, SolvingArguments * sargs, Arguments<ProgramArg> * pargs);
	~MSPSPTimeSATEncoding();


	SMTFormula * encode(int lb = INT_MIN, int ub = INT_MAX);

	SMTFormula * encode_sat(int makespan, int lb = INT_MIN, int precomputed_ub = INT_MAX);
