...
void parseMRCPSPSolution(istream & str,MRCPSP * instance,vector<int> &starts, vector<int> &modes){
	...
	for(int i=0;i<instance->getNActivities()+2;i++)
		starts[i] = readAssignment(str);

	modes.resize(instance->getNActivities()+2);
	for(int i=0;i<instance->getNActivities()+2;i++)
		modes[i] = readAssignment(str)-1;
}
...
MSPSP * parseMSPSP(const string & filename) {
  ...
  if(extension=="dzn" || extension=="DZN")
	  return parser::parseMSPSPfromDZN(filename);
  else {
	  cerr << "bad input file extension" << endl;
	  exit(BADFILEEXTENSION_ERROR);
	}
...
// Helper to skip until a specific character or handle MiniZinc syntax
void skipUntil(ifstream& f, char target) {
    char c;
    while (f.get(c) && c != target);
}

MSPSP* parseMSPSPfromDZN(const string& filename) {
    ifstream f(filename);
    if (!f.is_open()) {
        cerr << "Could not open file " << filename << endl;
        exit(BADFILE_ERROR);
    ...
    string line, word;
    int nActs = 0, nSkills = 0, nResources = 0, nPrecs = 0;

    // Preliminary pass to get dimensions or parse line by line
    while (f >> word) {
        ...
        if (word == "nActs") {
			...
			std::getline(f, word, ';');
			nActs = stoi(word);
		} else if (word == "nSkills") {
            skipUntil(f, '=');
			std::getline(f, word, ';');
			nSkills = stoi(word);
        } else if (word == "nResources") {
            skipUntil(f, '=');
			std::getline(f, word, ';');
			nResources = stoi(word);
        } else if (word == "nPrecs") {
			skipUntil(f, '=');
			std::getline(f, word, ';');
			nPrecs = stoi(word);
    ...
    // Reset file to read arrays now that we have dimensions
    f.clear();
    f.seekg(0);

    MSPSP* m = new MSPSP(nActs - 2, nResources, nSkills);

	vector<int> preds(nPrecs, -1);
	vector<int> sucs(nPrecs, -1);

	vector<vector<int>> demands(nActs, vector<int>(nSkills, -1));
	vector<int> durations(nActs, 0);
	vector<vector<bool>> mastery(nResources, vector<bool>(nSkills, false));

	bool scape = false;

	while (f >> word && !scape) {
		if (word[0] == '%') { getline(f, line); continue; }


        // Parse Skill Requirements (Matrix): sreq = [| 0,0... | 2,3... |];
        else if (word == "sreq") {
            skipUntil(f, '[');
			// Dummy activities too (0, n+1)
        ...
        // Parse Resource Mastery (Matrix): mastery = [| true, false... |];
        else if (word == "mastery") {
            skipUntil(f, '[');
            for (int r = 0; r < nResources; r++) {
                skipUntil(f, '|');
                for (int l = 0; l < nSkills; l++) {
                    std::getline(f, word, ',');
                    // Clean string and check for "true"
                    if (word.find("true") != string::npos)
                        mastery[r][l] = true;
                }
            }
        }

        // Parse Precedence (Arrays): pred = [...]; succ = [...];
        else if (word == "pred") {
            skipUntil(f, '[');
			for (int p = 0; p < nPrecs; p++) {
				if (p == nPrecs - 1)
					...
					std::getline(f, word, ',');
				int activity = stoi(word);
				preds[p] = activity;
			}
			// Read the rest of the line
			// And read one more to start with word == "succ"
			std::getline(f, line);
		}

		// Parse Succ (Arrays): pred = [...]; succ = [...];
        else if (word == "succ") {
            skipUntil(f, '[');
			for (int p = 0; p < nPrecs; p++) {
				std::getline(f, word, ',');
...
//	cout << endl;
	...
	for (int r = 0; r < nResources; r++) {
		for (int l = 0; l < nSkills; l++) {
			if (mastery[r][l])
				m->setResourceSkill(r, l);
		}
	}

	// Setting the activity durations
	for (int i = 0; i < nActs; i++) {
		m->setDuration(i, durations[i]);
	}
...
void parseMSPSPSolution(istream & str, MSPSP * instance, vector<int> &starts, vector<vector<pair<int,int>>> & assignments)
{
	...
	map<int, int> activityStartTimes;
	map<int, vector<pair<int,int>>> activityAssignments;
	...
	while (str >> token)
	{
		...
		{
			int activity, start;
			std::sscanf(token.c_str(), "S_%d:%d;", &activity, &start);
			activityStartTimes[activity] = start;
		}
		else if (tk == 'A')
		{
			int activity, resource, skill;
			std::sscanf(token.c_str(), "A:%d:%d:%d;", &activity, &resource, &skill);
			if (activityAssignments.find(activity) == activityAssignments.end())
			{
				activityAssignments[activity] = vector<pair<int,int>>();
			}
			activityAssignments[activity].push_back(pair<int,int>(resource, skill));
		}
		else
		{
			// Unrecognized token, possibly end of relevant data
			std::cerr << "Unrecognized token in solution: " << token << std::endl;
		}
	}

	if (activityStartTimes.size() != N + 2)
	{
		std::cerr << "Warning: Expected " << (N + 2) << " activities, but found " << activityStartTimes.size() << " in solution." << std::endl;
		std::exit(EXIT_FAILURE);
	}

	if (activityAssignments.size() != N)
	{
		std::cerr << "Warning: Expected assignments for " << (N) << " activities, but found " << activityAssignments.size() << " in solution." << std::endl;
		std::exit(EXIT_FAILURE);
	}
	...
	for (int i=0;i < N + 2; i++)
	{
		starts[i] = activityStartTimes[i];
	}

	assignments.resize(N + 2);
	for (int i = 1; i <= N; i++)
	{
		assignments[i] = activityAssignments[i];
	}
...
void parseMMKPSolution(istream & str,MMKP * instance,vector<int> &solution){
	...
	for(int i=0;i<instance->n;i++)
		solution[i] = readAssignment(str)-1;
}
...
int readAssignment(istream & str){
	findChar(str,':');
	int i;
	str >> i;
	return i;
